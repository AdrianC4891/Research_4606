...
if isReg
    ...
    chunk1Len = chunkLLen+pilotInd(blkLen);
    phData =  phEst(1)+(phEst(2)-phEst(1)).*(chunkLLen+1:chunk1Len)/chunk1Len;
    ...
    for idx = 2:numBlks
        ...
        phData = phEst(idx)+(phEst(idx+1)-phEst(idx)).*(1:pilotBlkFreq)/pilotBlkFreq;
        syncOut(stIdx:endIdx) = rxData(stIdx:endIdx).*exp(-1j*phData(:));
    end
    phData = phEst(end)+(arg4-phEst(end)).*(1:chunkLLen)/chunk1Len;
